// Quiz App JavaScript with Sound Effects
class QuizApp {
  constructor() {
  ...
  initializeSounds() {
    // Using Tone.js to create sound effects
    return {
      start: () => {
        ...
        setTimeout(() => synth.triggerAttackRelease("E5", "0.3"), 150);
        setTimeout(() => synth.triggerAttackRelease("G5", "0.4"), 300);
      },
  ...
  toggleSound() {
    ...
    this.showNotification(this.soundEnabled ? "Sound enabled" : "Sound muted");
  }

  init() {
    this.loadCategories();
    this.showSection("home");
    this.checkSharedDraft();
    this.addSoundToggle();
  }

  addSoundToggle() {
    const header = document.querySelector(".header .container");
    if (header && !document.querySelector(".sound-toggle")) {
  ...
  async startQuiz() {
    ...
    try {
      ...
      this.resetQuiz();
      this.playSound("start"); // Play start sound
  ...
  selectAnswer(selectedAnswer) {
    ...
    this.playSound("select"); // Play selection sound
    ...
    if (selectedAnswer === correctAnswer) {
      this.score++;
      isCorrect = true;
      setTimeout(() => this.playSound("correct"), 200); // Play correct sound
    } else {
      setTimeout(() => this.playSound("incorrect"), 200); // Play incorrect sound
    }
  ...
  endQuiz() {
    ...
    this.playSound("complete"); // Play completion sound
  ...
  checkSharedDraft() {
    // Check for draft in hash (e.g., #create?draft=...)
    const hash = window.location.hash;
    if (!hash.includes("?draft=")) return;
    ...
    try {
      draft = JSON.parse(decodeURIComponent(raw));
    } catch (e) {
